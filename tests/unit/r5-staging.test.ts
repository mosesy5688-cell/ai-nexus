...
describe('R5 stageCycle — ENABLED path (DI only)', () => {
    ...
    it('(RS-E6) STAGE ALONE is INSUFFICIENT for activation — dedup can be trusted only WITH exhaustive VERIFY', async () => {
        // A create-only 412 proves only that the KEY already EXISTS; it proves NOTHING about
        // the stored BYTES. So stageCycle never reports "verified" — it reports staged/deduped
        // and leaves proof to verifyCycleExhaustive (GET-and-rehash of EVERY blob), which is a
        // SEPARATE, independently fenced entrypoint. Activation requires BOTH wired together.
        const { manifest, bytesByLogical } = makeCycle('run-11-a1-abc', { 'meta-00.db': Buffer.from('SHARED') });
        const spy = makePutSpy(['data/blobs/' + manifest.blobs['meta-00.db']]);
        const res: any = await stageCycle({ mode: 'stage_enabled', cycleManifest: manifest, resolveBytes: (l: string) => bytesByLogical[l], putConditional: spy.put });
        expect(res.blobsDeduped).toBe(1);
        expect(res.staged).toBe(true);
        // STAGE never asserts verification of the pre-existing bytes.
        expect(Object.prototype.hasOwnProperty.call(res, 'verified')).toBe(false);
        // and the deduped blob was never re-downloaded/re-hashed by STAGE.
        expect(spy.calls.every((c: any) => c.ifNoneMatch === '*')).toBe(true);
    });
});
