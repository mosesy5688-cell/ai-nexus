...
import { REQUIRED_SINGLETONS, metaShardLogicals, assertManifestCensus, verifyCycleExhaustive, requiredRankingsDbs } from '../../scripts/factory/acceptance-staged-cycle.js';
...
function harness(metaShards: number, opts: any = {}) {
    ...
    // Producer authority (pack-db.js:97 unconditional shardWriter.open()): every real
    // cycle has fused-shard-000.bin, so the baseline harness carries it.
    if (!opts.noFused) put('fused-shard-000.bin', Buffer.from('F0'));
...
describe('R5 census invariant', () => {
    ...
    it('(AC-A5 / RED) a fused-shard GAP FAILS census', () => {
        const r = assertManifestCensus(harness(2, { extraBlobs: { 'fused-shard-002.bin': Buffer.from('F2') } }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_FUSED_GAP:1')).toBe(true);
    });
    it('(AC-A5b / RED) COMPLETE fused-set omission FAILS census (zero fused shards is NOT a valid cycle)', () => {
        // Producer authority: pack-db.js opens + finalizes shard 000 UNCONDITIONALLY, so a
        // manifest with NO fused shard at all is a complete-set omission, not an empty cycle.
        const r = assertManifestCensus(harness(2, { noFused: true }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_FUSED_SET_ABSENT')).toBe(true);
    });
    it('(AC-A5c / RED) shards_manifest.json omission FAILS census (required served singleton)', () => {
        expect(REQUIRED_SINGLETONS.includes('shards_manifest.json')).toBe(true);
        const r = assertManifestCensus(harness(2, { drop: ['shards_manifest.json'] }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_MISSING:shards_manifest.json')).toBe(true);
    });
    ...
    // ---- D-395 EXACT-10 rankings census (replaces the OBSOLETE four-type conditional rule)
    const allTen = () => Object.fromEntries(RANKINGS_DB_NAMES.map((n: string) => [n, Buffer.from('R:' + n)]));
    const rankingsOn = { rankings_dbs: true };

    it('(AC-A7) the required rankings set IS the canonical exact-10 authority, never type_counts-derived', () => {
        expect(RANKINGS_DB_COUNT).toBe(10);
        expect(requiredRankingsDbs(rankingsOn)).toStrictEqual([...RANKINGS_DB_NAMES]);
        expect(requiredRankingsDbs(rankingsOn).length).toBe(RANKINGS_DB_COUNT);
        // type_counts is IRRELEVANT to the requirement — all-zero counts still demand all 10.
        expect(requiredRankingsDbs({ rankings_dbs: true, type_counts: { model: 0, paper: 0, dataset: 0, tool: 0 } }))
            .toStrictEqual([...RANKINGS_DB_NAMES]);
        expect(assertManifestCensus(harness(2, { extraBlobs: allTen(), partitions: rankingsOn }).manifest))
            .toStrictEqual({ ok: true, failures: [] });
    });
    it('(AC-A7b / RED) dropping ANY ONE of the exact 10 rankings DBs turns the census RED', () => {
        for (const name of RANKINGS_DB_NAMES) {
            const r = assertManifestCensus(harness(2, { extraBlobs: allTen(), partitions: rankingsOn, drop: [name] }).manifest);
            expect(r.ok).toBe(false);
            expect(r.failures.includes(`CENSUS_MISSING:${name}`)).toBe(true);
        }
    });
    it('(AC-A7c / RED) partitions.rankings_dbs=true CANNOT coexist with a partial set', () => {
        const partial = { 'rankings-model.db': Buffer.from('RM'), 'rankings-paper.db': Buffer.from('RP') };
        const r = assertManifestCensus(harness(2, { extraBlobs: partial, partitions: rankingsOn }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.filter((f: string) => f.startsWith('CENSUS_MISSING:rankings-')).length).toBe(RANKINGS_DB_COUNT - 2);
    });
    it('(AC-A7d / RED) restoring the OBSOLETE four-type-only rule turns the census RED', () => {
        // The exact manifest the old rule ACCEPTED: rankings_dbs=true + type_counts with
        // dataset:0, carrying only the 3 non-empty rankings-<type>.db. Under the canonical
        // authority this is a 7-DB under-enumeration and MUST fail closed.
        const oldRuleShape = { 'rankings-model.db': Buffer.from('RM'), 'rankings-paper.db': Buffer.from('RP'), 'rankings-tool.db': Buffer.from('RT') };
        const oldRulePartitions = { rankings_dbs: true, type_counts: { model: 9, paper: 4, dataset: 0, tool: 2 } };
        const r = assertManifestCensus(harness(2, { extraBlobs: oldRuleShape, partitions: oldRulePartitions }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_MISSING:rankings-all.db')).toBe(true);
        expect(r.failures.includes('CENSUS_MISSING:rankings-dataset.db')).toBe(true);
        expect(r.failures.filter((f: string) => f.startsWith('CENSUS_MISSING:rankings-')).length).toBe(RANKINGS_DB_COUNT - 3);
    });
    it('(AC-A7e / RED) a FOREIGN/unknown rankings-*.db fails closed on identity', () => {
        const r = assertManifestCensus(harness(2, { extraBlobs: { ...allTen(), 'rankings-bogus.db': Buffer.from('X') }, partitions: rankingsOn }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_RANKINGS_FOREIGN:rankings-bogus.db')).toBe(true);
    });
    ...
    it('(AC-A9 / RED) rankings DBs present while UNDECLARED fail closed (no silent publication)', () => {
        const r = assertManifestCensus(harness(2, { extraBlobs: { 'rankings-model.db': Buffer.from('RM') } }).manifest);
        expect(r.ok).toBe(false);
        expect(r.failures.includes('CENSUS_RANKINGS_UNDECLARED:rankings-model.db')).toBe(true);
    });
});
