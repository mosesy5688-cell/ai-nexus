...
describe('resolveShardsForCandidates — fan-out-gated oracle await', () => {
    ...
    it('(d2) fan-out 1 (single-candidate model slug) + COLD index -> NO load, parity', async () => {
        ...
        const env = mockEnv(buildIndexBuffer([{ form: 'meta-llama--llama-3-8b', shardIdx: 11 }]));
    ...
    it('(e) fan-out <= 2 + index ALREADY WARM -> shrink applies, no extra load', async () => {
        // A prior high-fan-out request warmed the index in this isolate. A later
        // low-fan-out lookup may use it for free (no await, no new R2 GET).
        const env = mockEnv(buildIndexBuffer([{ form: 'arxiv--2307.01952', shardIdx: 7 }]));
    ...
    it('(e2) fan-out <= 2 + WARM index + all-miss -> absenceProven for free', async () => {
        // Warm index, low fan-out, every candidate absent: the all-miss verdict
        // is still available without any cold I/O.
        const env = mockEnv(buildIndexBuffer([{ form: 'arxiv--2307.01952', shardIdx: 7 }]));
