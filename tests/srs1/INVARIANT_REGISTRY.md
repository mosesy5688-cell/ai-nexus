...
| CYCLE-OUTPUT-A5 | A5 HYBRID manifest/guard membership consistency (D-2026-0706-285, PR-A): the `output/cache/**` walk is CLASSIFIED not raw-exact so the manifest can NEVER enumerate a member the uploader (r2-handoff `isUploadEligible`) refuses (the confirmed 3/4 producer read-back + 4/4 consumer verify blocker). (1) A consumer-REQUIRED small JSON < the 256B floor (`cache/search-manifest.json` search-worker-loader / `cache/fni-thresholds.json` master-fusion 4/4 / `cache/assertions/_summary.json` verify-assertions) is INCLUDED via the OPT-IN class-scoped `requiredJson` eligibility (valid+non-empty parseable JSON; a truncated/garbage/empty `{}` fails LOUD); (2) a regenerable `<payload>.meta.json` MD5 checksum sidecar is EXCLUDED by explicit OPTIONAL class (zero serve/4-4 readers); (3) an empty-`{}` (< 16B) `alt-by-category` frame is EXCLUDED by explicit EMPTY_PLACEHOLDER class -- DATA-SAFE because `knowledge-cache-reader.fetchCategoryAlts` returns `[]` for absent == present-empty (a >=16B real alt frame stays INCLUDED); (4) every INCLUDED member is asserted upload-eligible at generate (`MEMBER_UPLOAD_INELIGIBLE` fail-loud, never a late `FILE_MISSING`); (5) a non-JSON-family member fails LOUD `UNCLASSIFIED_MEMBER` at BOTH generate and verify; (6) EXACT-set read-back RETAINED. The SAME `isUploadEligible` is threaded (opt-in `requiredJson`, net-zero) through `r2-handoff.js` + the `r2-workflow-cli.js backup-dir --required-json` flag the finalize producer step opts into, so generate == upload eligibility by construction; the guard DEFAULT path + the 2/4 shards path stay BYTE-IDENTICAL (no blanket 256B floor lowering). NON-VACUITY: dropping the eligibility assertion reds F-A3/F-A6; un-excluding the sidecar reds F-A2; un-excluding the empty placeholder reds F-A4; restoring the unfiltered walk reds F-A5; a default-path floor change reds F-A12 | `scripts/factory/cycle-output-handoff-manifest.test.mjs` (F-A1..F-A12 + F-SRC/F-SRC2) | EXEC + SOURCE | **NEW** |
