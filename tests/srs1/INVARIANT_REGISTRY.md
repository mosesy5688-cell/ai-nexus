...
| MFH-GUARD-CONSISTENCY | PR-C (D-2026-0706-285): the fused/S1-BR `generateManifest` == the r2-handoff upload guard BY CONSTRUCTION for its GUARDED members. Per-member upload-path classification: a GUARDED `part-*.json.zst` reaches R2 via `backup-dir output/cache/fused/ --extensions=.json.zst,.complete` (`factory-upload.yml:617`; the guard CAN refuse it) so generate asserts it upload-eligible under the SAME `isUploadEligible` predicate (DEFAULT opts -- the zstd-magic + 16B floor) -> a sub-16B/no-magic part fails LOUD `MEMBER_UPLOAD_INELIGIBLE` at generate, never a late read-back `FILE_MISSING`. The `.complete` sentinel is BYPASS belt-and-suspenders: it is also uploaded via `upload-file … .complete` (`factory-upload.yml:621`) AND recovered via `restore-file …/.complete --strict` (`factory-upload.yml:744/902`), so it ALWAYS reaches R2 -> EXEMPT from the assert (a small non-`.zst` sentinel far below the 256B floor; asserting it would false-fail a member that uploads fine). Guarded predicate = `name.endsWith('.json.zst')`; the `.complete` bypass belt-and-suspenders is PRESERVED exactly. EXACT-set read-back (FILE_MISSING/FILE_EXTRA/size/sha/set_sha256) + `>=400` part + `>=400` processedShards + `.complete` gates PRESERVED; no guard relaxation, no producer change; `upload-eligibility.js` reused BYTE-IDENTICAL; `factory-upload.yml` NOT touched. NON-VACUITY: dropping the guarded-assert reds #G1 (generate SUCCEEDS on a sub-floor part); #G2 proves the bypass `.complete` is never fail-louded; #G3 a large real part is INCLUDED | `tests/unit/fused-handoff-manifest.test.ts` (#G1/#G2/#G3) | EXEC | **NEW** |
...
| VFS-PACK-DAG | Workflow DAG: vfs-pack-db produces attempt-scoped staging with ROLE-SPECIFIC R2 sub-prefixes (D-302/D-303): .db meta -> ${STAGING}meta/, .bin warm -> ${STAGING}warm/, term_index -> ${STAGING}term_index/ — each backup writes its OWN _manifest.json so NO TWO backup-dir share a manifest-bearing prefix (the meta restore index can never be overwritten by warm); meta data FIRST -> present RSS inputs -> manifest LAST -> descriptor LAST-of-all, set -e) + read-back verify + exports verified_vfs_pack_{staging_prefix,set_sha,producer_attempt}; VFS Derived consumes ONLY that identity, GHA exact-key fast path is verified ACCELERATION ONLY (restore-keys prefix authority REMOVED), verify-or-recover restores EACH role (meta/ + warm/ + term_index/) from its OWN R2 sub-prefix BEFORE the PRESERVED META>=1 fail-closed guard — R2 exact run-scoped staging is the correctness authority, never a fixed state/vfs-data/ recovery input; D-246 recovers every declared-present RSS input from the EXACT staging + fail-closed verify BEFORE rss-generator (declared-absent skipped); manifest/descriptor NEVER written into the public output/data/ tree; SCOPE: pack-db.js / sitemap-gen / rss-generator, workflow permissions (single top-level actions:write), timeouts UNCHANGED | `tests/unit/vfs-pack-handoff-workflow.test.ts` | CONFIG + SOURCE | **NEW** |
...
| VFS-DERIVED-GUARD-CONSISTENCY | PR-B (D-2026-0706-285): the SECONDARY sitemap/RSS generate == the r2-handoff upload guard BY CONSTRUCTION. The `vfs-derived-authority` carrier (`assertMemberEligibility`) asserts every INCLUDED member is upload-eligible under the SAME `isUploadEligible` predicate (DEFAULT opts -- the Final-Upload `backup-dir "$STAGE_LOCAL"` step passes NO `--required-json`): sitemap-index/child `.xml`/`.xml.gz` + present `rss/*.xml` on the 256B non-`.zst` floor. Every member is AUTHORITATIVE (NO empty-placeholder/optional EXCLUDE class: rss-generator SKIPS-EMPTY so a present feed's header alone is >256B; the sitemap index/children are corpus-large at the 550K 4/4 path). A member the guard would refuse (a present sub-256B `.xml`/`.gz` -- e.g. a degenerate SINGLE-child 246B sitemap index, only reachable at a tiny corpus, NEVER at the 4/4 recovery path) fails LOUD `MEMBER_UPLOAD_INELIGIBLE` at generate -- CONSISTENT with the DEFAULT guard already refusing it (NO guard relaxation, NO producer change; surfaces the pre-existing refusal early/loud instead of a late `FILE_MISSING`). The vfs-pack meta_db carrier has NO eligibility flag => its generate path is BYTE-IDENTICAL (D-245 unregressed). EXACT-set read-back + sitemap>=1/rss>=0 floors PRESERVED. NON-VACUITY: dropping the generate assert reds I1/I5 | `scripts/factory/vfs-derived-handoff-manifest.test.mjs` (I1/I5/I6/I7/I8-SRC) | EXEC + SOURCE | **NEW** |
...
> verified acceleration; R2 is the correctness authority. A5 REPAIR (D-2026-0706-285, PR-A): the
> `output/cache/**` walk is CLASSIFIED (not raw-exact) so the manifest can never enumerate a member
> the uploader refuses -- consumer-required small JSONs transport via class-scoped `requiredJson`
> eligibility, `.meta.json` checksum sidecars + empty-`{}` alt placeholder frames are EXCLUDED by
> explicit class, an unknown member fails loud -- closing the confirmed 3/4 read-back + 4/4 verify
> blocker (see CYCLE-OUTPUT-A5). TWO tiers: (1) the STATIC
...
| ID | Protected behavior | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| CYCLE-OUTPUT-MANIFEST | Manifest schema + verify (pure module, no R2): single carrier `cycle-output-authority` (prefix root `state/_handoff/cycle-output`, producer job `finalize`) with no manifest self-hash; per-file {relative_path,size_bytes,sha256}; set_sha256 over STABLE-SORTED (path,sha256) tuples; member root `cache` => CLASSIFIED `output/cache/**` set (A5, see CYCLE-OUTPUT-A5) with EXACT set equality (missing/extra => fail) + per-file size+sha256 + member_count agreement; required-class floors mesh_graph>=1 + search_core>=1 + category_stats>=1 (any of .json/.json.zst/.json.gz) + knowledge_index>=1 + trending>=1 + rankings>=1; NEVER count-only; `output/data/` is NEVER a member (4/4-owned); NO-REGISTRY-DOUBLE-BIND: a `registry/` segment or `global-registry*` file is EXCLUDED at generate (shared enumerator => same at verify) AND a manifest smuggling a registry member is REGISTRY_DOUBLE_BIND-rejected; symlink/traversal members REJECTED; the r2-handoff internal `_manifest.json` sidecar excluded at any depth | `scripts/factory/cycle-output-handoff-manifest.test.mjs` | EXEC | **NEW** |
| CYCLE-OUTPUT-A5 | A5 HYBRID manifest/guard membership consistency (D-2026-0706-285, PR-A + MF-1): the `output/cache/**` walk is CLASSIFIED not raw-exact so the manifest can NEVER enumerate a member the uploader (r2-handoff `isUploadEligible`) refuses (the confirmed 3/4 producer read-back + 4/4 consumer verify blocker). (1) A consumer-REQUIRED small JSON < the 256B floor (`cache/search-manifest.json` search-worker-loader / `cache/fni-thresholds.json` master-fusion 4/4 / `cache/assertions/_summary.json` verify-assertions) is INCLUDED via the OPT-IN **ADDITIVE / rescue-only** `requiredJson` eligibility -- reached ONLY after the base floor REFUSES a member, so it can NEVER block a member the base accepts: a `>=`floor member of ANY shape (`.gz`/`.jsonl`/`.ndjson`/`.json`/`.zst`) stays eligible EXACTLY as the base guard (never JSON-parsed), a sub-floor valid+non-empty non-`.zst` non-`.meta.json` JSON is RESCUED, and a sub-floor `.zst`/`.meta.json`/non-JSON stays blocked (a truncated/garbage/empty `{}` required JSON fails LOUD); (2) a regenerable `<payload>.meta.json` MD5 checksum sidecar is EXCLUDED by explicit OPTIONAL class (zero serve/4-4 readers); (3) an empty-`{}` (< 16B) `alt-by-category` frame is EXCLUDED by explicit EMPTY_PLACEHOLDER class -- DATA-SAFE because `knowledge-cache-reader.fetchCategoryAlts` returns `[]` for absent == present-empty (a >=16B real alt frame stays INCLUDED); (4) generate applies the SAME uploader opt (`backup-dir --required-json`) to EVERY included member (carrier `uploaderRequiredJson`) => generate == guard by construction: a sub-floor non-JSON / `.gz` / `.jsonl` authoritative member fails LOUD `MEMBER_UPLOAD_INELIGIBLE` at generate, never a late `FILE_MISSING`; (5) a non-JSON-family member fails LOUD `UNCLASSIFIED_MEMBER` at BOTH generate and verify; (6) EXACT-set read-back RETAINED. The SAME `isUploadEligible` is threaded (opt-in `requiredJson`, net-zero) through `r2-handoff.js` + `r2-bridge.js` (whole-opts passthrough) + the `r2-workflow-cli.js backup-dir --required-json` flag the finalize producer opts into; the guard DEFAULT path + the 2/4 shards path stay BYTE-IDENTICAL (no blanket 256B floor lowering; the `.zst` 16B / non-`.zst` 256B floors intact). NON-VACUITY: dropping the eligibility assertion reds F-A3/F-A6; un-excluding the sidecar reds F-A2; un-excluding the empty placeholder reds F-A4; restoring the unfiltered walk reds F-A5; a default-path floor change reds F-A12; the OLD REPLACING (non-additive) predicate reds MF1-A (a legit `>=`floor `.gz`/`.jsonl` refused); a dropped opts passthrough reds SF-2 | `scripts/factory/cycle-output-handoff-manifest.test.mjs` (F-A1..F-A12 + MF1-A/MF1-B + F-SRC/F-SRC2 + SF-2) | EXEC + SOURCE | **NEW** |
...
| AUTHW-SELF-STRIP-KILL | The vfs-derived + upload(FIX-4) consumers verify warm_read (verify-warm-read) and, on miss/mismatch, `rm -rf output/data/` then restore EACH producer role from its OWN R2 sub-prefix — `restore-dir "${STAGING_PREFIX}meta/"` (meta .db incl. meta-00.db) + `restore-dir "${STAGING_PREFIX}warm/"` (bins) + `restore-dir "${STAGING_PREFIX}term_index/"` (term_index) — each --strict fail-closed (exit 1); NO single WHOLE-prefix restore (D-302/D-303: role sub-prefixes give each backup its OWN _manifest.json, so the meta restore index can never be overwritten by warm). GHA cache is verified acceleration only; R2 exact run-scoped staging is the correctness authority. Reverting to the bare-prefix restore OR dropping a role restore reds the workflow-shape ties | `tests/unit/vfs-producer-authority-workflow.test.ts` | SOURCE | **NEW** |
| AUTHW-GUARD-CONSISTENCY | PR-B (D-2026-0706-285): AUTH-W warm-read generate == the r2-handoff upload guard BY CONSTRUCTION. `generateWarmReadManifest` asserts every INCLUDED member is upload-eligible under the SAME `isUploadEligible` predicate (DEFAULT opts -- the two producer `backup-dir` steps pass NO `--required-json`): the 3 bins (vector-core/hot-shard/id-index.bin) on the 256B non-`.zst` floor, `term_index/**` `.json.zst` on the 16B+zstd-magic floor. A member the guard would refuse (a degenerate sub-256B bin OR a sub-16B/no-magic term_index frame -- both CORRUPTION, never producer-legit; the vector/hot/id producers early-return with NO file on an empty corpus so the class floor -- not eligibility -- catches emptiness) fails LOUD `MEMBER_UPLOAD_INELIGIBLE` at generate, never a late read-back `FILE_MISSING`. EMPTY-BUCKET DETERMINATION: an empty-`{}` term_index frame CANNOT occur (inverted-index-builder emits a bucket ONLY for a prefix carrying >=1 term; high-freq chunks carry real postings; the manifest always has content), so there is NO empty-placeholder EXCLUDE class (the OPPOSITE of PR-A's alt-by-category frames) -- a sub-16B frame is corruption -> fail loud. EXACT-set read-back + class floors (vector/hot/id>=1, term_index_manifest>=1, term_index_bucket>=2) PRESERVED. NON-VACUITY: dropping the generate assert reds I2/I3/I4 | `scripts/factory/vfs-derived-handoff-manifest.test.mjs` (I2/I3/I4/I8-SRC/I9) | EXEC + SOURCE | **NEW** |
...
> rather than a hardcoded list. Discovery is now **STRUCTURAL (idiom-based), not
> naming-based**: an ADDITIVE cross-check enumerates EVERY `scripts/factory/*.{js,mjs}`
> (non-recursive, non-test) and flags the GENERATOR IDIOM (`carrier_type` schema field
> OR an exported `generateManifest()` OR `buildArchive()`+`archive_sha256`) — verified to
> match EXACTLY the 8 registered generators with ZERO false positives (r2-workflow-cli.js /
> migrate-fni-history-vault.js / master-fusion.js / verify-db.js / … carry none of these
> markers). Any idiom-matched file MUST be in the `/handoff/`-discovered set AND the
> registry, so a future in-class generator NOT named `*-handoff-*` (the review-proven
> naming escape) now FAILS. A NEW unregistered generator, a registry entry with no
...
| HG-STRUCTURAL | STRUCTURAL (filename-independent) idiom cross-check closing the naming-convention escape: the generator IDIOM (`carrier_type` OR exported `generateManifest()` OR `buildArchive()`+`archive_sha256`) over EVERY `scripts/factory/*.{js,mjs}` (non-recursive, non-test) matches EXACTLY the 8 registered generators with ZERO false positives; every idiom-matched file MUST be in the `/handoff/`-discovered set AND the registry. NON-VACUITY (review-proven escape, empirically verified RED): a scratch `foo-manifest.mjs` (idiom present, NO "handoff" in the name — GREEN under filename-only discovery) now REDS the cross-check ("carries the generator idiom but is NOT filename-discovered") | `tests/srs1/factory-handoff-guard-classification-invariant.test.ts` | SOURCE (STRUCT discovery) | **NEW** |
...
## Registry - C4-STAGE2-HF-TYPE-AUTHORITY (D-2026-0714-333, Stage 2)

> Producer + publication-safety invariant for the HF cross-type phantom fix.
> Auto-collected by the Tier-1 `unit-test` vitest job (no separate runner):
> `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts`.
> HERMETIC: pure functions + in-memory fixtures + repo workflow SOURCE; no
> network / DB / R2 / live HuggingFace.

| ID | Protected behavior | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| C4-S2-TYPE-AUTHORITY | The HF MODEL adapter's canonical identity type is the SOURCE FAMILY (`model`), NEVER a dataset minted from relationship metadata (`cardData.datasets` / `pipeline_tag==='dataset'`) - the phantom root cause. inferType is DEMOTED to a descriptive/validation signal only. SURGICAL dataset-only demotion: the legacy `tool` classification is PRESERVED so NO existing `hf-tool--*` canonical id changes (D-333 S10 valid_id_changed_count=0). The immutable INTERNAL `source_entity_type` anchors the canonical-id mint at BOTH chokepoints (`registry-manager.js:104`, `merge-batches.js` export) via a shared `resolveIdentityType` (family-first, tool-preserving, legacy-passthrough - no canonical-ID/UMID algorithm change). `source_entity_type` is INTERNAL ONLY: dropped by slim projection and absent from the packed meta-row builder (never a public REST/MCP/SDK/OpenAPI/FNI/ranking/packed field). The model<->tool disagreement is recorded as an informational diagnostic only, on a DIFFERENT axis from the demoted model<->dataset case. NON-VACUITY (implementer-run RED-then-restore): M1 cardData.datasets->model, M2 pipeline_tag=dataset->model, M3 tool preserved, M4 mint-from-family, M5 resolver table, M12 internal-only. | `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts` | EXEC + SOURCE | **NEW** |
| C4-S2-PHANTOM-RECONCILE | Narrow, evidence-gated, fail-closed cross-type reconciliation. DELETION ORACLE = the exact current attempt's completeness-gated HF-model + HF-dataset source authorities (tuple-bound {run id, attempt, source role, head SHA, authority-set hash, completeness, generated-at UTC, member count}); live HF / author listings are CORROBORATION ONLY, never in the deletion path. The LOCKED all-of predicate is the ONLY deletion basis; deletable classes = ONLY `PROVEN_MODEL_ONLY_PHANTOM_DATASET` + `PROVEN_DATASET_ONLY_PHANTOM_MODEL`. NEVER-alone evidence (cardData.datasets / shared slug / 401/403/404/429/timeout/5xx / absence-from-incomplete-attempt / absence-from-live-author-listing / absence-from-sitemap / low pop/FNI / old last_modified) can never by itself delete. GENUINE_DUAL retains both; PRIVATE/AUTH-GATED retains; CURRENT_SOURCE_ABSENT_STALE_RECORD + SOURCE_RENAMED_OR_DELETED are NOT deleted (`stale_record_removed=0`); incomplete/other-attempt/hash-invalid authority OR any UNCLASSIFIED_CONFLICT/publication-blocking transient => `C4_STAGE2_SOURCE_FAMILY_AUTHORITY_INCOMPLETE` = ZERO_PUBLICATION. Every excluded phantom is recorded in an immutable run-bound removal ledger {removed canonical id, removed UMID, false type, surviving valid typed id, source-authority hashes, evidence class, reason code, run id+attempt, implementation SHA, observed/generated UTC}; count contract asserts removed==ledger members, unclassified=0, valid_id_changed=0, valid_umid_changed=0, stale_record_removed=0. Publication safety: master-fusion applies the manifest as a narrow `allValidIds` set-difference (registry untouched, append-only); factory-upload runs BOTH fail-closed gates - PRE-fusion (manifest present+bound+complete) and POST-pack (no proven phantom survives the packed set + count reconciliation). NON-VACUITY (implementer-run RED-then-restore): M6/M6b proven phantoms, M7 never-alone, M8 genuine dual, M9 incomplete-authority-as-absence, M10 author-listing-negative, M11 reconcile-all-current-cycle-absent, M12b private-gated, gate-i PRE-fusion, gate-ii POST-pack. | `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts` | EXEC + SOURCE | **NEW** |

## Registry - C4-STAGE2-CANDIDATE-UNIVERSE (D-2026-0714-334/335/336)

> Amends the C4-S2 reconciliation to a CANDIDATE-UNIVERSE attempt-bound census. Two Tier-1
> hermetic files (both vitest auto-collected): `c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts`
> (M1-M12 + type authority) and `c4-stage2-candidate-universe-census.test.ts` (M13-M21 + census
> pagination + freeze/drift + production-path).

| ID | Protected behavior | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| C4-S2-MODEL-ALWAYS | HF MODEL source canonical identity type is ALWAYS 'model' (never 'tool'/'dataset'); HF DATASET always 'dataset'; inferType (incl. 'tool') is DESCRIPTIVE only and never controls identity type / ID prefix / registry key. Transformers-no-pipeline HF model now mints hf-model-- (prospective model->tool prevention); existing hf-tool-- rows NOT rewritten (valid_id_changed_count=0), counted report-only (model_tool_residual_count), G1-P OPEN. NON-VACUITY: M3, M5, M16 RED-then-restore. | `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts` + `tests/srs1/c4-stage2-candidate-universe-census.test.ts` | EXEC | **NEW** |
| C4-S2-CANDIDATE-UNIVERSE | Deletion (the two PROVEN_*_PHANTOM model<->dataset classes ONLY) requires: a CLOSED candidate universe frozen from the carried registry (HF collisions + type-mismatch rows) with a deterministic member+universe hash (pure `c4s2-candidate-universe.js`); census REQUESTED; EACH candidate owner's model AND dataset listing exhausted via real Link: rel="next" cursor to no-next-link (adapters; NO top-N truncation; ANY mid-pagination 429/5xx/timeout/loop/endpoint-jump => INCOMPLETE, partial NEVER usable); completeness written EXACTLY COMPLETE_FOR_C4_STAGE2_CANDIDATE_UNIVERSE (never global); model+dataset+universe share ONE run/attempt/head tuple; Process/Aggregate/Upload verify that tuple stage-by-stage; NO post-freeze out-of-universe collision (drift) at the final pre-publication scan. THREE-WAY: (i) not requested => dormant/publishable; (ii) requested+complete+verified => classify+delete, fail-closed on unclassified/residual/drift; (iii) requested-but-incomplete/tuple-mismatch/drift => ZERO_PUBLICATION (keep serving previous good cycle). NON-VACUITY: M6-M21 RED-then-restore + a production-path test driving freeze->adapter pagination->authority->classifier->gate. | `tests/srs1/c4-stage2-candidate-universe-census.test.ts` + `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts` | EXEC + SOURCE | **NEW** |

### D-2026-0715-338 amendment - fail-closed cascade tuple + fail-closed harvest selection (2 blockers)

> Two reachable fail-opens survived the D-337 hardening. **Blocker 1 (missing/failed/corrupt/foreign tuple must be
> ZERO_PUBLICATION):** the pure `resolveCascadeTuple` `!t` branch returned `{ok:true, tuple:null}` (fail-OPEN) and the
> two factory-upload census-skip guard steps did `if ! restore ...; then echo ...; exit 0; fi` (fail-OPEN) - so on a
> fused/pack cache hit a tuple restore FAILURE (R2 flake / corrupt / absent) skipped the PRE/POST exclusion+drift+count
> gates and proceeded to publish. Design truth: the producer ALWAYS emits a tuple+manifest every cycle (Harvest always
> classifies + uploads the manifest; Process/Aggregate restore `--strict` and re-emit the tuple unconditionally), so a
> consumer-side missing/unreadable tuple = broken cascade. Fix: `resolveCascadeTuple` `!t` => `{ok:false, terminal:
> CASCADE_TUPLE_MISSING}`; a new pure `censusSkipGuardVerdict({resolve, censusRequested, skipCompute})` returns the
> terminal on `!resolve.ok` (missing/foreign/malformed) OR on `censusRequested && skipCompute` (CENSUS_SKIP_FORBIDDEN),
> else `{ok:true, dormant}` - the census-skip guards call it and BOTH the strict tuple restore and the `MREF=$(...)`
> command substitution now fail the step (`|| exit 1` + `[ -n "$MREF" ]`) on absent/foreign/malformed (all 4 upload
> gate sites hardened). Dormant is a validated manifest with `censusRequested=false`, NEVER tuple absence. **Blocker 2
> (manual dispatch forced attempt=1 / auto-selected latest):** factory-process get-run-id used `gh run list --limit 1`
> (latest, no operator attempt identity) + defaulted `HARVEST_RUN_ATTEMPT=1`, so a failed attempt-1 manifest could be
> replayed as a deletion authority. Fix: a `run_attempt` dispatch input; pure `resolveHarvestSelection` (workflow_run
> authoritative via id+run_attempt; manual REQUIRES both run_id AND run_attempt - NO latest, NO default-1 => fail-closed)
> + pure `assertHarvestAttemptEligible` (manual path re-verifies via `gh api .../attempts/<n>` that the operator-named
> run/attempt is the EXACT Factory-1/4 Harvest attempt on main that concluded success at THIS head; all conditions
> AND-gated). BEHAVIORAL (helper-driven, not YAML string-grep) + RED-then-restore.

| ID | Protected behavior (blocker) | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| C4-S2-D338-TUPLE-FAILCLOSED | **BLOCKER 1**: a missing / R2-restore-error / foreign-head / malformed cascade tuple on a fused/pack cache hit => ZERO_PUBLICATION (never the OLD `ok:true tuple:null` no-op nor the census-skip `exit 0`); a valid tuple with `censusRequested=false` on a cache hit => allowed dormant; a valid tuple + census + cache hit => CENSUS_SKIP_FORBIDDEN (PRE/POST gates would skip). Driven end-to-end through `resolveCascadeTuple` THEN `censusSkipGuardVerdict` (the exact upload census-skip guard decision). WORKFLOW SOURCE: both census-skip guards + all 4 MREF sites in factory-upload.yml fail closed on absent/foreign/malformed. NON-VACUITY (implementer-run RED-then-restore): reverting the `!t` fail-close reddens [BL4-XCAS] + [BL1-D338 missing]; removing the census+skip check reddens [BL1-D338 valid+census+cache]. | `tests/srs1/c4-stage2-hf-type-authority-and-phantom-reconciliation.test.ts` (BEHAVIORAL) + `tests/srs1/c4-stage2-candidate-universe-census.test.ts` (WF-CONTRACT SOURCE) | EXEC + SOURCE | **NEW** |
| C4-S2-D338-HARVEST-SELECT | **BLOCKER 2**: harvest run/attempt selection is fail-closed - workflow_run is authoritative (runId+run_attempt); a MANUAL dispatch REQUIRES both run_id AND run_attempt (NO `gh run list` latest, NO default attempt 1) or `resolveHarvestSelection` => `{ok:false, C4S2_HARVEST_SELECTION_INCOMPLETE}`; the operator-named attempt must be the EXACT successful Factory-1/4 Harvest attempt on main at THIS head (`assertHarvestAttemptEligible` AND-gates id/run_attempt/name/head_branch/conclusion==success/head_sha, else `C4S2_HARVEST_ATTEMPT_INELIGIBLE`). A `run_attempt` dispatch input exists; factory-process get-run-id no longer contains `gh run list --workflow factory-harvest.yml` or a default-1. NON-VACUITY (implementer-run RED-then-restore): reverting to default-attempt-1 manual reddens [BL2-SEL manual-missing-attempt/run_id]; removing the conclusion==success check reddens [BL2-ELIG conclusion!=success]. | `tests/srs1/c4-stage2-candidate-universe-census.test.ts` (BEHAVIORAL + WF-CONTRACT) | EXEC + SOURCE | **NEW** |

## Registry - RANKINGS-DB-AUTHORITY (rankings artifact authority repair)

> Deterministic, hermetic lock for the EXACT `rankings-<group>.db` publication set that
> `POST /api/v1/select` and the catalog list fast path depend on. **ROOT CAUSE:** the 10
> DBs reached the 4/4 packer ONLY through the `cycle-<run>-output` GHA cache (cache WRITES
> are 100% denied on this repo), the 3/4 recovery read a MUTABLE FIXED prefix
> (`state/satellite-rankings/*`) with 10 hardcoded names, every restore `|| true` and the
> count merely echoed, and `pack-finalizer.js` derived `manifest.partitions.rankings_dbs`
> from an any-one-file `.some()` boolean inside a swallowing `try {} catch {}` - so the key
> was never written and `/api/v1/select` returned 503. **FIX:** two dedicated attempt-scoped
> R2 carriers (one per cross-stage seam), a COMPLETE-SET integrity/identity verifier of
> record, and TWO mandatory fail-closed gates before any public write. TWO tiers: (1) the
> STATIC workflow + source-invariant locks `tests/srs1/rankings-db-authority-invariant.test.ts`
> + `rankings-db-seam-b-publication-invariant.test.ts` + `rankings-db-verifier-source-invariant.test.ts`
> (all vitest auto-collected; shared text fixtures in `tests/srs1/rankings-db-wf-fixtures.ts`, split
> to stay under the CES Art 5.1 250-line ceiling); (2) the hermetic `node --test` contract suite
> `scripts/factory/rankings-db-handoff-manifest.test.mjs`, wired into the SAME required
> `unit-test` job via the C2-LOCK explicit `node --test` list. R2 I/O reuses the BYTE-FROZEN
> GENERIC `r2-workflow-cli` subcommands (no new subcommand, no transport edit).

| ID | Protected behavior | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| RANKINGS-DB-SET-10 | The rankings publication set is an EXACT 10-name set (`RANKINGS_DB_NAMES`), never a count and never `.some()`: a MISSING member, an EXTRA `rankings-*.db` (e.g. a retired `rankings-prompt.db`), a zero-byte member, a member smuggled in through the manifest, or a manifest declaring != 10 rankings members ALL fail with a machine code (`RANKINGS_DB_SET_MISSING` / `RANKINGS_DB_SET_EXTRA` / `DB_EMPTY` / `MANIFEST_MEMBER_COUNT_MISMATCH`). Enforced at FOUR places: exporter emission (`RANKINGS_GROUP_EMPTY` / `RANKINGS_GROUP_UNEXPECTED` / `RANKINGS_DB_COUNT_MISMATCH`, naming the offending groups AND the per-group counts), carrier `generate` + `verify` (both carriers, `exactDbSet`), `pack-finalizer` (throws), and the pre-publish gate. NON-VACUITY: dropping `checkExactDbSet` reds (S2)/(S13); restoring the any-one-file `.some()` shortcut reds (S5)+(S15)+#16; a count>=10 check instead of set equality reds (S13); an empty group silently `continue`d reds (S2c). | `scripts/factory/rankings-db-handoff-manifest.test.mjs` (S2/S2b/S2c/S5/S13/S15) + `tests/srs1/rankings-db-seam-b-publication-invariant.test.ts` (#16) + `tests/srs1/rankings-db-verifier-source-invariant.test.ts` (#22/#23) | EXEC + SOURCE | **NEW** |
| RANKINGS-DB-IDENTITY-RUN-ATTEMPT-HEAD | Every rankings DB carries its cycle identity in `site_metadata` (`factory_run_id` / `factory_run_attempt` / `head_sha`, additive TEXT alongside `rankings_group` / `entity_count` / `generated`; `entity_count` meaning + TYPE UNCHANGED because `src/utils/catalog-fetcher.js` live-reads it for pagination). All 10 members must agree (`IDENTITY_INCONSISTENT`), must equal the expected 3/4 run id (`IDENTITY_RUN_MISMATCH`) and head sha (`IDENTITY_HEAD_MISMATCH`), the attempt must be a positive int not from the future (`IDENTITY_ATTEMPT_FUTURE`), and `rankings_group` must equal the FILENAME group (`GROUP_METADATA_MISMATCH`). The exporter REFUSES to write a DB when the identity env is missing/malformed (`RANKINGS_IDENTITY_ENV_INVALID`) - never an empty identity a later gate would reject. The 4/4 cross-workflow consumer, which cannot know the 3/4 head sha on its own, gets an EXACT head-SHA bind by family-binding the `rankings-db` descriptor to its `cycle-output` sibling (same run + attempt + head, both written by the SAME finalize execution). NON-VACUITY: (S3a) foreign run/head; (S3b) ONE foreign member among 10; (S3c) future attempt; (S3f) sibling head/run/attempt disagreement; (S7b) missing identity env; (S12) swapped group metadata. Dropping the sibling bind lets a foreign-head rankings set promote at 4/4 and reds (S3f)+#13. The sibling reader accepts `producer_run_id ?? finalize_run_id`; (H4) PINS which key the FROZEN cycle-output carrier actually supplies (`finalize_run_id`, and NOT `producer_run_id`) in its required-field list AND in the producing workflow step, so a frozen-shape change surfaces as a test failure instead of being silently absorbed by the fallback -- and proves a sibling carrying NEITHER spelling is `SIBLING_FIELD_MISSING` while a foreign id under the frozen spelling still reds. | `scripts/factory/rankings-db-handoff-manifest.test.mjs` (S3a-S3f/S7b/S12) + `tests/srs1/rankings-db-authority-invariant.test.ts` (#7) + `tests/srs1/rankings-db-seam-b-publication-invariant.test.ts` (#13) | EXEC + SOURCE | **NEW** |
| RANKINGS-DB-SEAM-A-EXACT | SEAM_A (3/4 `aggregate-rankings` -> 3/4 `finalize`) has EXACT authority: the producer establishes `state/_handoff/rankings-satellite/<run>/attempt-<attempt>/` bound to run id + attempt + head_sha, ordered DB-verification -> manifest -> data FIRST -> `manifest.json` LAST -> `handoff.json` LAST-OF-ALL -> descriptor read-back + `verify-descriptor` -> producer FULL-SET read-back with set-SHA equality, and only THEN announces the authority. The carrier covers BOTH the 10 DBs AND the `cache/rankings/**` page JSONs + `category_stats` (floors `rankings_db` = 10, `ranking_pages` >= 10, `category_stats` >= 1) so a complete DB set can NEVER mask missing ranking JSON pages. The consumer is descriptor-first, runs UNCONDITIONALLY (the old `if: cache-hit != 'true'` gate is GONE - a GHA hit is not evidence of correctness), verifies the GHA accelerator against the SAME manifest, DISCARDS it on any mismatch and re-fetches from the EXACT prefix, promotes by verified COPY (never rename/move) after WIPING BOTH promotion targets (`output/data/rankings-*.db` AND `output/cache/rankings/**` -- a MERGE would let an ORPHAN page from a previous attempt, e.g. a stale `p40.json.zst` after `totalPages` shrank, ride unverified into the cycle-output carrier; the authority carries the COMPLETE page set so wiping is safe), then RE-VERIFIES the WHOLE promoted PROJECTION at the destination via `verify-projection` -- every authority member (10 DBs + every ranking page + `category_stats`) present and byte-identical, plus ZERO orphan inside each EXCLUSIVE SCOPE; out-of-scope workspace siblings (`output/data/meta.db`, the rest of `output/cache/**`) are correctly ignored. The 10-hardcoded-name, `|| true`, count-only fixed-prefix block is DELETED. **CORRECTION (superseded by D-395 item 5 - see `RANKINGS-SATELLITE-PREFIX-RETIRED` below):** this row originally recorded that `state/satellite-rankings/*` "survives ONLY as a WRITE (historical/diagnostic)". That is NO LONGER TRUE - the three remaining FUTURE writes were subsequently DELETED, so the prefix now has **ZERO command positions (write AND read)** in both workflows and the immutable attempt-scoped carrier is the only rankings carrier for correctness AND diagnostics. Do NOT "restore" the diagnostic write: it reds `#10` and re-introduces the ~510 PUT/cycle the Founder ordered removed. NON-VACUITY: (S11) 10 DBs + zero page JSONs => `REQUIRED_CLASS_BELOW_FLOOR`; (S9) a missing/malformed descriptor => `DESC_*` fail-loud; (S3e) a fixed / `attempt-latest` / attempt-stripped / two-level prefix => `DESC_PREFIX_MISMATCH`; #4 reds if the hardcoded group loop returns; #10 reds if the fixed prefix appears in ANY command position, read OR write (its detector is hoisted into one named function shared by the real scan and its own self-proof, so narrowing the scan reds the proof); #8 reds if a cache-hit gate is reintroduced; #6 reds on any commit-order regression; (H1a) reds if the page wipe reverts to a merge (planted orphan `p40.json.zst` => `PROJECTION_ORPHAN_MEMBER`); (H1b) reds if the destination re-verify covers only the DB half (missing/same-length-tampered page + missing `category_stats`); #9b reds if the wipe/copy/verify ORDER regresses. | `tests/srs1/rankings-db-authority-invariant.test.ts` (#4-#10/#10b) + `scripts/factory/rankings-db-handoff-manifest.test.mjs` (S1/S3e/S9/S11) | CONFIG + SOURCE + EXEC | **NEW** |
| RANKINGS-DB-SEAM-B-EXACT | SEAM_B (3/4 `finalize` -> 4/4 `vfs-pack-db`) has EXACT authority via a SECOND dedicated carrier `state/_handoff/rankings-db/<3of4-run-id>/attempt-<attempt>/` (a separate descriptor is REQUIRED: different producer job than SEAM_A, and 4/4 resolves the 3/4 run id independently via `check-upstream` upstream-run-id). The cycle-output carrier stays BYTE-FROZEN and still NEVER carries `output/data/`. The producer mirrors the canonical commit-last ordering + full-set read-back. The 4/4 consumer resolves descriptor -> family-binds the cycle-output sibling -> manifest -> `restore-dir --strict` into `/tmp/rankings-stage/` (NEVER a new `output/` subtree, because `r2-upload-s3` `R2_PREFIX_FILTER` includes bare `data/`) -> full check set on the STAGED copies -> wipe + verified COPY into `output/data/rankings-<g>.db` -> RE-VERIFY at the destination. It runs UNCONDITIONALLY (no `if:`, no `skip_compute` / `force_fresh` gate), AFTER the `intra-4-4-vfs-pack` cache restore (whose `restore-keys` PREFIX can carry a previous attempt's tree) and BEFORE skip-detection, the Packer, and the end-of-job vfs-pack handoff manifest that captures `output/data/*.db` for the FIX-4 publication binding. NON-VACUITY: (S1)/(S7) prove the R2-only path is sufficient with ZERO GHA content; (S14) proves promotion is byte-identical and idempotent; #14 reds if the step is gated or reordered after the Packer/handoff; #13 reds if `|| true`, a rename/move, an `attempt-latest` glob, or a dropped sibling bind appears; #25 reds if the frozen transport/cycle-output carriers are edited or `pack-db.js` grows. | `tests/srs1/rankings-db-seam-b-publication-invariant.test.ts` (#11-#15) + `tests/srs1/rankings-db-verifier-source-invariant.test.ts` (#25) + `scripts/factory/rankings-db-handoff-manifest.test.mjs` (S1/S7/S10a/S10b/S14) | CONFIG + SOURCE + EXEC | **NEW** |
| RANKINGS-DB-FAILCLOSED-PREPUBLISH | `manifest.partitions.rankings_dbs = true` is the RESULT of a complete-set verification, and any incomplete state fails closed BEFORE any public write. TWO mandatory gates: (a) `pack-finalizer.js` calls the THROWING `assertRankingsDbSet(shardDir, verifyOptsFromEnv())` and only then writes the flag as EXPLICIT boolean `true` (never `false`, never inside a swallowing `try/catch`); the throw precedes the `shards_manifest.json` write, so `pack-db.js` fails -> `vfs-pack-db` fails -> `vfs-derived` + `upload` (which `needs:` it) never run -> ZERO public write and the last-good published build keeps serving. (b) a NEW `upload`-job gate sits strictly between `Publication-Family Closure Gate` and `Capacity Preflight - Final Upload`, i.e. BEFORE the SOLE `node scripts/factory/r2-upload-s3.js` public CDN write - MANDATORY because `skip_compute=true` skips `pack-db.js` entirely and restores `output/data/` from a `restore-keys` prefix, making (a) bypassable. The gate re-resolves the 3/4 authority descriptor + manifest itself and asserts the EXACT 10-set + size/sha256 + SQLite magic + `PRAGMA quick_check` + producer-DERIVED schema + provenance + identity + `partitions.rankings_dbs === true`, else `exit 1`. An ABSENT `rankings_dbs` key (the live production shape) and a `false`/truthy-non-boolean value are BOTH refused - an absent key is NEVER a verified negative. NO SILENT DEGRADE: every workflow verify call site sets `RANKINGS_REQUIRE_IDENTITY=1`, under which an EMPTY `RANKINGS_EXPECT_RUN_ID`/`RANKINGS_EXPECT_HEAD_SHA`/`RANKINGS_MAX_ATTEMPT` THROWS `IDENTITY_BINDING_REQUIRED_BUT_EMPTY` instead of meaning "skip that check" -- so a lost `$GITHUB_OUTPUT` write can never quietly downgrade the gate from identity-bound to structure-only. The flag deliberately does NOT require `RANKINGS_DB_MANIFEST` (the producer pre-checks run before it exists). SINGLE-LINE STDOUT CONTRACT: `verify-descriptor` emits EXACTLY one tab-delimited line (prefix, set_sha, head_sha, attempt) with all diagnostics on stderr, and every consumer cuts fields from `DESC_LINE=$(... | tail -n1)` so a future extra stdout line cannot turn a prefix into a multi-line value. `src/pages/api/v1/select.ts`'s 503 protection is UNTOUCHED. NON-VACUITY: (S2b) a partial set makes `finalizePack` throw AND leaves NO `shards_manifest.json`; (S6) a complete set yields boolean `true`; (S6b) a foreign expected run id throws even on an intact complete set; (S6c) absent/`false`/`1`/`'true'` all refused; #17 reds if the gate is moved after the publish step or the publish site count changes; #18 reds if any gate branch stops exiting 1; #20 reds if the 503 guard is weakened; (H2) reds if an empty expectation is silently accepted under the flag; (H2b) proves a structurally-perfect complete set STILL fails `finalizePack` when the required binding is empty; (H3) reds if stdout gains a second line, the field order changes, a diagnostic moves to stdout, or a call site cuts raw `$OUT`; #15b reds if any rankings-verifying step drops the flag. | `scripts/factory/rankings-db-handoff-manifest.test.mjs` (S2b/S6/S6b/S6c) + `tests/srs1/rankings-db-seam-b-publication-invariant.test.ts` (#16-#20) | EXEC + SOURCE + CONFIG | **NEW** |
| RANKINGS-DB-CONSTANT-SINGLE-SOURCE | The rankings group set has exactly ONE declaration: `src/constants/rankings-groups.js` (plain ESM `.js`, NOT `.ts`, because the Node factory scripts cannot import TypeScript - same rationale as `src/constants/shard-constants.js`), exporting the frozen ordered 10-member `RANKINGS_GROUPS`, `RANKINGS_DB_COUNT`, `RANKINGS_DB_NAMES`, `rankingsDbName()` (throws on an unknown group) and `rankingsGroupFromDbName()` (returns null rather than inventing a group). `rankings-db-exporter.js`, `rankings-generator.js`, the carrier `.mjs` and `rankings-db-verifier.js` all IMPORT it and re-declare nothing; the verifier's schema expectations (tables, the 34 `entities` columns, the 5 indexes) are PARSED from the producer's `RANKINGS_SCHEMA` so producer and verifier cannot drift; the 5 category slugs stay in parity with the frontend `CATEGORY_SLUGS`; and NO workflow hardcodes the group names (the `for t in all model paper dataset tool ...` loop is DELETED). NON-VACUITY: #1 reds on a changed/unfrozen constant; #2 reds if the category slugs diverge from the frontend list; #3 reds if any consumer re-declares the list; #4 reds if a workflow reintroduces a hardcoded loop or a fixed-prefix per-name restore; #10b reds if any new step branches on `cache-hit`, adds a `restore-keys` prefix, or decides by count; (A1)/(A3) red on a group-count change or a schema drift; (S4d) reds when a real DB's index/column set diverges from the derived expectation. | `tests/srs1/rankings-db-authority-invariant.test.ts` (#1-#4) + `tests/srs1/rankings-db-verifier-source-invariant.test.ts` (#21/#24) + `scripts/factory/rankings-db-handoff-manifest.test.mjs` (A1/A3/S4d) | SOURCE + EXEC | **NEW** |

### D-395 amend - publication-manifest sibling + pack-cache version bump (BLOCKING-1)

> Founder ruling on PR #2297: merge NOT authorized, AMEND REQUIRED; direction (i) APPROVED,
> direction (ii) (weakening D9b) REJECTED. **Confirmed failure chain on the pre-amend head:**
> `VFS_PACK_CODE_VERSION` unchanged from base -> an OLD pack cache can hit -> `pack-db.js` is
> SKIPPED -> `pack-finalizer.js` (the ONLY writer of `partitions.rankings_dbs=true` AND of
> `output/data/shards_manifest.json`) never runs -> the FIX-4 recovery does
> `rm -rf output/data/` and restores ONLY `.db` / warm `.bin` / `term_index`, never
> `shards_manifest.json` -> D9b requires that file -> publication refused on the
> skip-compute and FIX-4 paths. **Fix:** (1) bump the pack-cache token so the first
> post-merge run is necessarily a FRESH pack, and (2)+(3) carry the manifest as an ADDITIVE
> verified sibling of the D-245 attempt-scoped vfs-pack authority, recovered in BOTH FIX-4
> consumer paths BEFORE any public write. **D9b is byte-unchanged in semantics: boolean
> `true` is still required, and the manifest is never fabricated or boolean-patched.**
> SCOPE NOTE: implemented WITHOUT editing `vfs-derived-handoff-manifest.mjs` (the D-245
> carrier) -- the sibling logic lives in `rankings-db-handoff-manifest.mjs` and is composed
> by the workflow, so the existing `.db` member classification and set hashing are untouched.

| ID | Protected behavior | Assertion file | Evidence | Status |
|----|--------------------|----------------|----------|--------|
| RANKINGS-PACK-VERSION-BUMP | `VFS_PACK_CODE_VERSION` is the NEW frozen token `rankings-manifest-authority-v1`; the retired token appears NOWHERE (so an old cache key cannot be reconstructed); the token scopes EVERY `intra-4-4-vfs-pack` cache key AND restore-prefix with no bare cross-version fallback; skip-compute still requires `sentinel == version` and a fresh pack still stamps the sentinel. Consequence: an old pack cache can never satisfy skip-detect, so `pack-finalizer` (the only writer of the flag + the manifest) always runs on the first post-merge cycle. NON-VACUITY (implementer-run mutation): reverting the token to `citation-authority-v2` reds #S1 AND the pre-existing `vfs-pack-cache-version` frozen-token case (M1). | `tests/srs1/rankings-db-publication-sibling-invariant.test.ts` (#S1-#S3) + `tests/unit/vfs-pack-cache-version.test.ts` | CONFIG + SOURCE | **NEW** |
| RANKINGS-PUBLICATION-SIBLING | `output/data/shards_manifest.json` is carried as an ADDITIVE sibling at `${STAGING}publication/shards_manifest.json`, bound by `descriptor.shards_manifest_sha256` + `descriptor.shards_manifest_size`, and DELIBERATELY NOT mixed into the `.db` set hash (the meta `set_sha256` and the `warm_read` sibling stay byte-identical; the `--ext=.db` generate call is unchanged and never sees the manifest). Producer ORDER is enforced: exists -> non-empty -> parses -> `partitions.rankings_dbs === true` -> hash+size -> data uploaded FIRST -> descriptor LAST -> **durable descriptor read-back PARSED for the identity** -> **EXACT agreement with the locally computed hash+size (own `DESC_SHARDS_MANIFEST_{SHA,SIZE}_DIVERGED` codes)** -> sibling object read back and verified WITH THE DESCRIPTOR-PARSED VALUES -> only THEN "authority established". **LOAD-BEARING BINDING (Founder finding on the first amend):** presence+syntax alone left the binding DECORATIVE - a different-but-well-formed 64-hex SHA and a different-but-positive size passed every check, because the sibling was then verified against the PRODUCER-LOCAL shell values and those same locals were exported as the job outputs, so the durable descriptor could declare a FALSE identity with no consequence. The enforced identity now ORIGINATES in the read-back: `local file -> descriptor -> durable read-back -> parsed descriptor identity -> durable sibling verification -> job outputs -> both FIX-4 consumers`. An EMPTY parsed value is a HARD error, never "no expectation", and the LOCAL identity is itself non-empty-guarded at the call site BEFORE it seeds the descriptor or the expectation. **RATCHET NOTE - READ BEFORE "FIXING" 4 RED TESTS.** The module's comparison branch is deliberately SKIPPED when no expectation is supplied (`expect.sha256`/`expect.size` empty or absent), and that legacy-permissive branch is RETAINED ON PURPOSE for off-workflow callers. `(D5)`, `(D6)`, `(D7)` and `(D8)` ASSERT that the no-expectation branch still accepts - `(D8)` **depends** on it to prove that the pre-fix syntax-only contract would have accepted the lying descriptor, which is what makes the closed defect a permanent regression test. Therefore making the expectation MANDATORY inside the module is a **deliberate 4-test edit, not a regression**: whoever does it must update those four tests in the same change and say why. It is NOT required for pipeline safety - the binding is already unconditional in the pipeline because the workflow (i) always passes `--expect-sha`/`--expect-size` and (ii) hard-fails on an empty local identity before doing so, both pinned by `#S6b`. The module-level default is a compatibility surface, not the enforcement point. Validation is SEMANTIC as well as byte-level, through the SAME `verifyPublishedFlag` D9b uses, so a syntactically valid but semantically wrong manifest can never become an authority. NON-VACUITY: (D1) absent/`false`/truthy-non-boolean flag refused; (D2) missing/empty/unparseable refused; (D3) TRUNCATED and SAME-LENGTH-TAMPERED (still-valid-JSON, still-flag-true) both refused - the tamper fixture is byte-length-identical so ONLY the hash can catch it; (D4) hash vs size mismatches reported distinctly; (D5) descriptor missing EITHER field (absent OR empty-string) refused, malformed values refused; (D7) a valid-FORMAT but WRONG sha and a valid-POSITIVE but WRONG size each DIVERGE (and the test additionally asserts the OLD syntax-only contract WOULD have accepted them, pinning that the gap was real); (D8) THE COUNTEREXAMPLE - local file and durable sibling byte-identical and semantically valid while the DESCRIPTOR declares a well-formed false identity - is refused, and is shown to be unsatisfiable from either direction; (D9) three-way agreement (local file == descriptor == durable sibling) passes end-to-end. Implementer-run mutations: dropping `shards_manifest_sha256` from the descriptor reds #S5 (M3); replacing the read-back re-verify with a non-verifying call reds #S5 (M4); dropping the SHA divergence gate reds (D7)+(D8) (N1); dropping the SIZE divergence gate reds (D7) (N2); not passing the expectation from the workflow reds #S6b (N3); reverting the job outputs to the producer-local values reds #S6+#S6b (N4); verifying the durable sibling with the locals reds #S6b (N5); removing the read-back empty-value hard guard reds #S6b (N6); re-binding `RB_SM_SHA`/`RB_SM_SIZE` to the producer-local after the parse reds #S6b. That is enforced NOT by enumerating spellings but by two structural locks: the single assignment line must EQUAL its canonical form verbatim, and those two names may appear on NO executable line other than the explicitly allow-listed READ sites - which also catches `=`-less mechanisms (`read -r`, `printf -v`, `declare`, `eval`, indirect expansion). Eight spellings were exercised as EXAMPLES, not as the coverage argument: newline-braced, same-line, command-substitution, intermediate variable, indirect expansion `${!V}`, echo|tr pipeline, `read -r` and `printf -v` (B1-B8); each is caught by the name census. NOTE the earlier regex-pair these replaced was bypassable six ways (a `[^$
]*` blacklist cannot cross a `$`, and both locks keyed on `=`). The VALUE guarantee itself rests on the divergence gate, not on these syntactic locks. Deleting the local non-empty guard reds #S6b (N8); disabling the hash comparison reds (D3)+(D4) (M8); relaxing `!== true` to falsy reds #S9+(D1) (M7). | `scripts/factory/rankings-db-handoff-manifest.test.mjs` (D1-D5) + `tests/srs1/rankings-db-publication-sibling-invariant.test.ts` (#S4-#S6/#S10) | EXEC + SOURCE | **NEW** |
| RANKINGS-PUBLICATION-SIBLING-RECOVERY | BOTH FIX-4 consumer paths (`vfs-derived` AND the Final Upload `upload` job) verify-or-recover the sibling: local `output/data/shards_manifest.json` is compared against the authority hash+size, and MISSING **or** hash-mismatch **or** unparseable **or** `partitions.rankings_dbs !== true` all count as "must recover"; recovery is `restore-file --strict` from the EXACT attempt prefix followed by a RE-VERIFY. In `upload` the step sits strictly AFTER the FIX-4 step that does `rm -rf output/data/` and strictly BEFORE both the D9b gate that reads the file and the sole public CDN write. Fail-closed only: no `|| true`, no `continue-on-error`, >=3 `exit 1` branches per step, and NO fabrication/boolean-patching of the manifest (no `rankings_dbs: true` literal, no `jq` edit) - the only thing that can satisfy the step is the producer-verified authority copy. END-TO-END: after a simulated FIX-4 erase + sibling recovery, the manifest + the EXACT 10 DBs + one identical run/attempt/head are all consistent and the D9b contract passes. NON-VACUITY: disabling the upload-side recovery step reds #S7+#S8 (M2); removing the post-recovery re-verify reds #S7 (M5); (D6) proves the FIX-4 erase genuinely yields `SHARDS_MANIFEST_ABSENT` before recovery and PASS after. | `tests/srs1/rankings-db-publication-sibling-invariant.test.ts` (#S7-#S9) + `scripts/factory/rankings-db-handoff-manifest.test.mjs` (D6) | CONFIG + SOURCE + EXEC | **NEW** |
| RANKINGS-SATELLITE-PREFIX-RETIRED | The mutable fixed prefix `state/satellite-rankings/**` has ZERO command positions in EITHER workflow - no write AND no read. Its three FUTURE writes (`backup-dir .../rankings/`, the `category_stats` `upload-buffer`, and the 10-DB parallel `upload-buffer` loop) are DELETED, so the immutable attempt-scoped rankings-satellite authority is the only rankings carrier for BOTH correctness and diagnostics and the ~510 PUT/cycle duplication is gone (the carrier becomes a RELOCATION, roughly net-neutral, not a duplication). NO existing R2 object is deleted and NO GC/delete behaviour is introduced in this PR; the retired prefix may still be NAMED in the root-cause narrative (comments only, asserted). The lock is kept NON-VACUOUS: it proves its own detector fires on synthetic write AND read command lines rather than being trivially satisfied. NON-VACUITY (implementer-run mutation): re-adding a single `backup-dir ... state/satellite-rankings/rankings/` command reds #10 (M6). | `tests/srs1/rankings-db-authority-invariant.test.ts` (#10) | SOURCE | **AMENDED** |
