...
| GAP-5b-EXPLICIT-MEMBERSHIP | The prepared-entity-data `cache/` root is CLASSIFIED not unfiltered: a proven optional-accelerator (`entity-checksums`/`daily-accum`/`fni-history`) is EXCLUDED (name-class, invariant to bytes/etag), an UNKNOWN cache file fails LOUD `UNCLASSIFIED_MEMBER` at BOTH generate and verify (consistent by construction), and NO `cache/` file enters the authority (net members = the `data/` set only); the stale 11B `entity-checksums` no longer affects `set_sha256`. NON-VACUITY: restoring the unfiltered walk reds F-T1/F-T5; an etag/content-based skip reds F-T10 | `scripts/factory/shards-handoff-manifest.test.mjs` (F-T1/F-T5/F-T6/F-T10) | EXEC + SOURCE | **NEW** |
