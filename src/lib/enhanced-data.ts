...
export async function getModelsWithCache(kv?: KVNamespace) {
    ...
    if (cached) {
        console.log('âœ?Cache HIT: models list');
        return cached;
    }

    console.log('â?Cache MISS: models list - fetching and caching');
...
export async function getRelatedModels(
    ...
    const cached = await getCache(kv, cacheKey);
    if (cached) {
        console.log(`âœ?Cache HIT: related models for ${modelId}`);
        return cached;
    }

    console.log(`â?Cache MISS: related models for ${modelId} - computing`);
