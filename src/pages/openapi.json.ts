...
const SEARCH_DESC_CATALOG = 'Full-text search across the Free2AITools catalog of AI models, tools, datasets, papers, and benchmarks, ordered by a relevance score in which the FNI is the largest single component. The response is not ordered by the `fni_score` field it returns, and the result set is bounded. Returns up to 20 results per request.' + PAGINATION_NOTE + TRANSIENT_NOTE;

function injectCount(phrase: string | null): string {
    if (!phrase) return SEARCH_DESC_CATALOG;
    return `Full-text search across the Free2AITools catalog of ${phrase} AI models, tools, datasets, papers, and benchmarks, ordered by a relevance score in which the FNI is the largest single component. The response is not ordered by the \`fni_score\` field it returns, and the result set is bounded. Returns up to 20 results per request.${PAGINATION_NOTE}${TRANSIENT_NOTE}`;
}
