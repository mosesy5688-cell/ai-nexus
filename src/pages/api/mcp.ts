...
const TOOLS = [
    {
        ...
        description: 'Keyword discovery over the Free2AITools catalog of AI models, datasets, papers, and tools. Returns matching catalog entries (metadata) ordered by a relevance score based on the FNI (Free2AITools Nexus Index) and, where term-match data is available, how well the entry matches the query; the FNI is a 5-factor score: Semantic relevance, Authority, Popularity, Recency, Quality. The response is not ordered by the fni_score field it returns, and the result set is bounded. The Semantic factor is a query-time baseline, not a live per-entity measurement (fni_s is returned null with a note). USE WHEN you need to discover which AI entities exist for a topic or keyword. DO NOT USE for general web search, to run/call/execute a model, to get a generated or inferred answer, or to route to an inference provider — this returns catalog metadata only, for the calling agent to reason over and decide on. Free discovery catalog: results are never paid placement / sponsored, and there is no billing or payment. Read-only, no side effects. May return a retryable transient 503 under cold-path or fallback budget limits; retry according to Retry-After. Use free2aitools_select_model instead when you have specific hardware or license constraints.',
        inputSchema: {
            type: 'object',
            properties: {
