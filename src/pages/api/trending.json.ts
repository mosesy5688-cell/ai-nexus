 ...
 * 
 * Returns top models by Velocity (fni_v) with Quality Gates:
 * - fni_score >= 30 (Quality threshold)
 * - downloads > 100 (Noise filter)
 */
...
export const GET: APIRoute = async ({ locals }) => {
    try {
        ...
        // Prevents "garbage" from appearing on homepage
        const query = `
            SELECT 
            ...
            FROM models 
            WHERE fni_v > 0 
              AND fni_score >= 30   -- Quality Gate
              AND downloads > 100   -- Noise Filter
            ORDER BY fni_v DESC 
            LIMIT 8
        `;
