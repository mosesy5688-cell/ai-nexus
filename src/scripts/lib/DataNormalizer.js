...
export const DataNormalizer = {
    normalize(item, defaultType = 'model') {
        ...
        const normalized = {
            ...
            fni_s: item.fni_s ?? item.fni_metrics?.s ?? 50.0,
            fni_a: item.fni_a ?? item.fni_metrics?.a ?? 0,
            fni_p: item.fni_p ?? item.fniP ?? item.fni_metrics?.p ?? 0,
            fni_r: item.fni_r ?? item.fni_metrics?.r ?? 0,
            fni_q: item.fni_q ?? item.fni_metrics?.q ?? 0,
