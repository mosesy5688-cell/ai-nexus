...
export function renderPaletteResults(results, container) {
    ...
    container.innerHTML = results.map((r, i) => `
      <div class="result-item" data-index="${i}" onclick="window.location.href='/${escapeHtml(r.type)}/${escapeHtml(r.slug)}'">
        <div class="w-8 h-8 rounded bg-white/5 flex items-center justify-center text-[10px] font-bold text-white/40 shrink-0">
          ${escapeHtml(String(r.type || '').substring(0, 2).toUpperCase())}
        </div>
        <div class="flex-1 min-w-0 pr-2">
          <div class="flex items-center justify-between pb-1">
            <div class="flex items-center gap-2 min-w-0 flex-1 pr-4">
              <span class="text-sm font-medium text-white/90 truncate">${escapeHtml(r.name)}</span>
              ${r.author ? `<span class="px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[9px] border border-blue-500/20 truncate max-w-[100px] shrink-0">${escapeHtml(r.author)}</span>` : ''}
              ${r.task ? `<span class="px-1 py-0.5 rounded bg-white/5 text-[9px] truncate max-w-[80px] shrink-0">${escapeHtml(r.task)}</span>` : ''}
            </div>
          ...
          <div class="hidden sm:flex items-center gap-2 text-[10px] text-white/40 font-mono overflow-hidden whitespace-nowrap">
             <span class="truncate flex-1 max-w-[140px] text-white/30" title="${escapeHtml(r.slug)}">${escapeHtml((r.slug || '').split('/').pop() || r.slug)}</span>
             ...
             ${r.license ? `<span class="w-1 h-1 rounded-full bg-white/10 shrink-0"></span><span class="text-white/50 shrink-0 truncate max-w-[80px]">${escapeHtml(r.license)}</span>` : ''}
