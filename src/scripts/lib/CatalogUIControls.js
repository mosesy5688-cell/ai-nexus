...
export class CatalogUIControls {
    static setupInfiniteScroll(instance) {
        ...
        console.log(`[CatalogUIControls] ðŸš€ Setting up infinite scroll for ${instance.config.gridId}`);
        ...
        instance.observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting) {
                console.log('[CatalogUIControls] ðŸ‘€ Sentinel in view.');
                if (!instance.source.isLoadingShard && instance.hasMore()) {
                    console.log('[CatalogUIControls] ðŸ“¥ Triggering loadMore...');
                    instance.loadMore();
                } else {
                    console.log(`[CatalogUIControls] â­ï¸ Skipping loadMore: loading=${instance.source.isLoadingShard}, hasMore=${instance.hasMore()}`);
                }
            }
