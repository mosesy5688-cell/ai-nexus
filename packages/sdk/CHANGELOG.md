...
## [0.1.0] — 2026-06-28 (first version)
...
- Packaging hardening (no runtime/API behavior change): added MIT `LICENSE`
  ...
  longer references unshipped `src/`.
- Publish-prep metadata: added `publishConfig.access: public` (scoped public
  package), `author`, and a defensive `prepublishOnly` build hook.
...
- Package name `@free2aitools/sdk` is CONFIRMED; the npm org `free2aitools` is
  ...
  published** (publication pending Founder authorization); the manifest is
  publish-prepared (`publishConfig.access: public`).
