...
def load_entities_from_r2(client, bucket):
    """Load entity data from R2 cache files."""
    entities = []
    
    # Try to load trending.json for model entities
    try:
        ...
        print(f"  📊 Loaded {len(data.get('models', []))} models from trending.json")
    except Exception as e:
        print(f"  ⚠️ Could not load trending.json: {e}")
