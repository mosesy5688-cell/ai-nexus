 ...
 *
 *   PRESENT_VALID  (protocol==1 AND >=1 monitored conserved+complete summary)
 ...
 *       -> FAIL + publication blocked. The ONLY new blocking case.
 *   NOT_ACTIVE_OR_NOT_APPLICABLE  (legacy/JS/no monitored shard/zero shards)
...
export function foldShard(agg, acc) {
    ...
    if (v.reason === 'engine_path_not_applicable') return v; // legacy JSON shard
...
export function finalizeCanary(cap, agg, expectedShardCount) {
    ...
    // (missing/old/non-conserved summary, aggregate non-conservation, or
    //  drop-detail incomplete). The ONLY new blocking case.
    ...
    if (agg.monitoredSummaries === 0) {
        // v1-capable, shards processed, yet NOT ONE monitored binary summary.
        return done(base, 'EXPECTED_BUT_MISSING', 'FAIL', true, 'no_monitored_summary');
    }
    ...
    // ── PRESENT_VALID: conserved + complete. dropped gates PASS vs DEGRADED. ──
    if (agg.dropped === 0) {
