...
async function main() {
    ...
    const shardFileZst = path.join(CONFIG.CACHE_DIR, `merged_shard_${shardId}.json.zst`);
    const shardFileGz = path.join(CONFIG.CACHE_DIR, `merged_shard_${shardId}.json.gz`);
    const shardFilePath = await fs.stat(shardFileZst).then(() => shardFileZst).catch(() => shardFileGz);
    let shardEntities = [];
